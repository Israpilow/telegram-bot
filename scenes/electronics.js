...
startStep.on('text', async (ctx) => {
  try {
    ...
    await ctx.replyWithHTML(
      '<b>ШАБЛОН ДЛЯ ЗАПОЛНЕНИЯ ЭЛЕКТРОНИКИ</b>\n\n1. Название объявления <i>( Например: Iphone 11 )</i>\n\n2. Описание объявления <i>( Например: Iphone 11 в идеальном состоянии. Покупался два года назад )</i>\n\n3. Цена <i>( Например: 40 000р )</i>\n\n4. Адрес <i>( Например: г.Махачале ул.Акушинского 90б )</i>\n\n5. Контакты <i>( Например: 89898966666 )</i>'
    );
...
async function sendImages(ctx) {
  ...
  await ctx.telegram.sendMediaGroup(5583727224, photos[ctx.from.id]);
...
const electronicsScene = new Scenes.WizardScene(
  'electronicsWizard',
  startStep,
  photoStep,
  botStep,
