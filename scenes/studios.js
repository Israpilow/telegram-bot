...
startStep.on('text', async (ctx) => {
  try {
    ...
    await ctx.replyWithHTML(
      '<b>ШАБЛОН ДЛЯ ЗАПОЛНЕНИЯ СТУДИИ</b>\n\n1. Название объявления <i>( Например: Сдается студия )</i>\n\n2. Описание объявления <i>( Например: В идеальном состоянии. Темплый пол. Есть Смарт Тв телевизор )</i>\n\n3. Цена <i>( Например: 15 000р )</i>\n\n4. Адрес <i>( Например: г.Махачале ул.Акушинского 90б )</i>\n\n5. Контакты <i>( Например: 89898966666 )</i>'
    );
...
const studiosScene = new Scenes.WizardScene(
  'studiosWizard',
  startStep,
  photoStep,
  botStep,
