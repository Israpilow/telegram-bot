...
nameStep.action('appliances_btn', async ctx => {
    try {
        ...
        await ctx.replyWithHTML('<b>ШАБЛОН ДЛЯ ЗАПОЛНЕНИЯ ДЛЯ ДОМА И ДЛЯ ДАЧИ</b>\n\n<i>1. Название объявления\n2. Состояние(новое|б/у)\n3. Описание объявления\n4. Цена\n5. Адрес\n6. Контакты\n7. Стоимость объявления 200 руб.</i>');        await ctx.replyWithHTML('Сбербанк 5228 6005 2216 8428');
        return ctx.wizard.next()
    } catch (e) {
        console.log(e)
    }
...
conditionStep.on('photo', async ctx => {
    try {
        ...
        await ctx.telegram.sendMessage(1954192936, `<b>ДЛЯ ДОМА И ДЛЯ ДАЧИ</b>\n\n${wizardData.checkPhoto}`, {
            parse_mode: "HTML"
        });
        await ctx.telegram.sendMessage(974900206, `<b>ДЛЯ ДОМА И ДЛЯ ДАЧИ</b>\n\n${wizardData.checkPhoto}`, {
            parse_mode: "HTML"
        });
...
const appliancesScene = new Scenes.WizardScene('appliancesWizard', startStep, nameStep, checkPhotoStep, photoStep, conditionStep)
